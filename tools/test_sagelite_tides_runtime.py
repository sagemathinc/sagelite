...
def _load_source_desolvers():
    fullname = "sage.calculus.desolvers"
    path = ROOT / "src" / "sage" / "calculus" / "desolvers.py"
...
def test_tides_compile_flags_fall_back_to_sage_local(monkeypatch):
    ...
    desolvers = _load_source_desolvers()

    assert desolvers._tides_compile_flags() == (
