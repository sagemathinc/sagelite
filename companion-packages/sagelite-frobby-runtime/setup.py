...
def _candidate_bindirs() -> list[Path]:
    dirs = []
    for variable in ("SAGELITE_FROBBY_BINDIR", "FROBBY_BINDIR"):
...
def _find_executable() -> Path:
    for bindir in _candidate_bindirs():
        candidate = bindir / "frobby"
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return candidate.resolve()
    searched = "\n  ".join(os.fspath(path) for path in _candidate_bindirs())
    raise RuntimeError(
        "could not find a frobby executable. "
        "Set SAGELITE_FROBBY_BINDIR to the Sage-built bin directory.\n"
        f"Searched:\n  {searched}"
    )
...
def _runtime_libraries(executable: Path) -> list[Path]:
    ...
    for line in output.splitlines():
        ...
        if name.startswith(("libgmpxx", "libgmp")) and path != "not":
...
class build_py(_build_py):
    def run(self):
        ...
        target = Path(self.build_lib) / "sagelite_frobby" / "data" / "bin"
        lib_target = Path(self.build_lib) / "sagelite_frobby" / "data" / "lib"
        shutil.rmtree(target, ignore_errors=True)
        shutil.rmtree(lib_target, ignore_errors=True)
        target.mkdir(parents=True, exist_ok=True)
        lib_target.mkdir(parents=True, exist_ok=True)

        shutil.copy2(source, target / "frobby-real")
        wrapper = target / "frobby"
        wrapper.write_text(
            "#!/bin/sh\n"
            'HERE="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"\n'
            'LD_LIBRARY_PATH="$HERE/../lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"\n'
            "export LD_LIBRARY_PATH\n"
            'exec "$HERE/frobby-real" "$@"\n'
        )
        wrapper.chmod(0o755)
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            plat_name = os.environ.get("SAGELITE_FROBBY_RUNTIME_PLAT_NAME")
            if plat_name:
