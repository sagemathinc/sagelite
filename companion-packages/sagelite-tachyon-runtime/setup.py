...
def _candidate_bindirs() -> list[Path]:
    dirs = []
    for variable in ("SAGELITE_TACHYON_BINDIR", "TACHYON_BINDIR"):
...
def _find_executable() -> Path:
    for bindir in _candidate_bindirs():
        candidate = bindir / "tachyon"
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return candidate.resolve()
    searched = "\n  ".join(os.fspath(path) for path in _candidate_bindirs())
    raise RuntimeError(
        "could not find a tachyon executable. "
        "Set SAGELITE_TACHYON_BINDIR to the Sage-built bin directory.\n"
        f"Searched:\n  {searched}"
    )
...
class build_py(_build_py):
    def run(self):
        ...
        target = Path(self.build_lib) / "sagelite_tachyon" / "data" / "bin"
        lib_target = Path(self.build_lib) / "sagelite_tachyon" / "data" / "lib"
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            plat_name = os.environ.get("SAGELITE_TACHYON_RUNTIME_PLAT_NAME")
            if plat_name:
