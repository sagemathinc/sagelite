...
def _candidate_bindirs() -> list[Path]:
    dirs = []
    for variable in ("SAGELITE_KISSAT_BINDIR", "KISSAT_BINDIR"):
...
def _find_bindir() -> Path:
    ...
    raise RuntimeError(
        "could not find a Kissat executable directory containing "
        f"{', '.join(REQUIRED_PROGRAMS)}. Set SAGELITE_KISSAT_BINDIR to the "
...
class build_py(_build_py):
    def run(self):
        bindir = _find_bindir()
        target = Path(self.build_lib) / "sagelite_kissat" / "data" / "bin"
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            plat_name = os.environ.get("SAGELITE_KISSAT_RUNTIME_PLAT_NAME")
            if plat_name:
