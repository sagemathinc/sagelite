from __future__ import annotations

import os
from pathlib import Path
import sys


def bin_prefix() -> str:
    """
    Return the bundled Kissat executable directory with a trailing separator.
    """
    return os.fspath(Path(__file__).resolve().parent / "data" / "bin") + os.sep


def executable_path(program: str = "kissat") -> Path:
    return Path(__file__).resolve().parent / "data" / "bin" / program


def run_program(program: str) -> int:
    executable = executable_path(program)
    if not executable.is_file():
        raise RuntimeError(
            f"Kissat executable is missing from companion package: {program}"
        )
    os.execv(os.fspath(executable), [program, *sys.argv[1:]])
    return 127


def kissat() -> int:
    return run_program("kissat")
