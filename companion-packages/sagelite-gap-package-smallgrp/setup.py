...
def _candidate_gap_roots() -> list[Path]:
    roots = []
    roots.extend(_split_roots(os.environ.get("SAGELITE_GAP_SMALLGRP_ROOT")))
...
def _candidate_package_dirs() -> list[Path]:
    dirs = []
    for variable in (
        "SAGELITE_GAP_SMALLGRP_PACKAGE_DIR",
        "SAGELITE_GAP_PACKAGE_SMALLGRP_DIR",
    ):
    ...
    for root in _candidate_gap_roots():
        pkg = root / "pkg"
        dirs.extend(pkg.glob("smallgrp*"))
        dirs.extend(pkg.glob("SmallGrp*"))
    return dirs


def _find_smallgrp_package_dir() -> Path:
    ...
    raise RuntimeError(
        "could not find a GAP SmallGrp package directory containing PackageInfo.g. "
        "Set SAGELITE_GAP_SMALLGRP_ROOT or SAGELITE_GAP_SMALLGRP_PACKAGE_DIR.\n"
        f"Searched:\n  {searched}"
    )
...
class build_py(_build_py):
    def run(self):
        source = _find_smallgrp_package_dir()
        target = (
            Path(self.build_lib)
