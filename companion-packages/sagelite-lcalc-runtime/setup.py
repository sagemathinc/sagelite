...
def _candidate_bindirs() -> list[Path]:
    dirs = []
    for variable in ("SAGELITE_LCALC_BINDIR", "LCALC_BINDIR"):
...
def _find_executable() -> Path:
    for bindir in _candidate_bindirs():
        candidate = bindir / "lcalc"
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return candidate.resolve()
    searched = "\n  ".join(os.fspath(path) for path in _candidate_bindirs())
    raise RuntimeError(
        "could not find an lcalc executable. "
        "Set SAGELITE_LCALC_BINDIR to the Sage-built bin directory.\n"
        f"Searched:\n  {searched}"
    )
...
class build_py(_build_py):
    def run(self):
        ...
        target = Path(self.build_lib) / "sagelite_lcalc" / "data" / "bin"
        lib_target = Path(self.build_lib) / "sagelite_lcalc" / "data" / "lib"
        shutil.rmtree(target, ignore_errors=True)
        shutil.rmtree(lib_target, ignore_errors=True)
        target.mkdir(parents=True, exist_ok=True)
        lib_target.mkdir(parents=True, exist_ok=True)

        shutil.copy2(source, target / "lcalc-real")
        wrapper = target / "lcalc"
        wrapper.write_text(
            "#!/bin/sh\n"
            'HERE="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"\n'
            'LD_LIBRARY_PATH="$HERE/../lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"\n'
            "export LD_LIBRARY_PATH\n"
            'exec "$HERE/lcalc-real" "$@"\n'
        )
        wrapper.chmod(0o755)
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            plat_name = os.environ.get("SAGELITE_LCALC_RUNTIME_PLAT_NAME")
            if plat_name:
