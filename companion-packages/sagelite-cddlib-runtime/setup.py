...
PROGRAMS = ("cddexec", "cddexec_gmp", "redcheck_gmp", "scdd", "scdd_gmp")


def _candidate_bindirs() -> list[Path]:
    dirs = []
    for variable in ("SAGELITE_CDDLIB_BINDIR", "CDDLIB_BINDIR"):
...
def _find_programs() -> dict[str, Path]:
    ...
    if missing:
        ...
        raise RuntimeError(
            "could not find required cddlib executables "
            f"{', '.join(missing)}. Set SAGELITE_CDDLIB_BINDIR to the "
...
def _runtime_libraries(executable: Path) -> list[Path]:
    ...
    for line in output.splitlines():
        ...
        if name.startswith(("libcdd", "libgmp")) and path != "not":
...
class build_py(_build_py):
    def run(self):
        ...
        target = Path(self.build_lib) / "sagelite_cddlib" / "data" / "bin"
        lib_target = Path(self.build_lib) / "sagelite_cddlib" / "data" / "lib"
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            plat_name = os.environ.get("SAGELITE_CDDLIB_RUNTIME_PLAT_NAME")
            if plat_name:
