...
def _candidate_bindirs() -> list[Path]:
    dirs = []
    for variable in ("SAGELITE_BENZENE_BINDIR", "BENZENE_BINDIR"):
...
def _find_executable() -> Path:
    for bindir in _candidate_bindirs():
        candidate = bindir / "benzene"
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return candidate.resolve()
    searched = "\n  ".join(os.fspath(path) for path in _candidate_bindirs())
    raise RuntimeError(
        "could not find a benzene executable. "
        "Set SAGELITE_BENZENE_BINDIR to the Sage-built bin directory.\n"
        f"Searched:\n  {searched}"
    )
...
class build_py(_build_py):
    def run(self):
        ...
        target = Path(self.build_lib) / "sagelite_benzene" / "data" / "bin"
        shutil.rmtree(target, ignore_errors=True)
        target.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target / "benzene")
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            plat_name = os.environ.get("SAGELITE_BENZENE_RUNTIME_PLAT_NAME")
            if plat_name:
