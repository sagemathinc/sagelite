...
Podman had no active container, and a process scan excluding the probe shell
found zero external Sagelite, cibuildwheel, or authoritative-run matches. No
...
`post64` rebuild. The root filesystem had 17,241,296,896 bytes free, and the
separate `/mnt/cocalc` volume had 63,296,004,096 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,750,361,088 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
`9511474fe3d4f6f158ab2905a34dacae918efe4c`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 09:31 UTC

Read-only reconciliation at `2026-07-27T09:31:45Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
both durable service names were not found and inactive. Docker was absent,
Podman had no active container, and a literal process scan found zero
external Sagelite, cibuildwheel, or authoritative-run matches. No result was
inferred, no remote state was changed, and no duplicate build was launched.

The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,251,004,416 bytes free, and the
separate `/mnt/cocalc` volume had 63,311,142,912 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,744,323,584 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`236a23fa9d8246c508e1a93989d0b5b8ea8dff6e`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 09:01 UTC

Read-only reconciliation at `2026-07-27T09:01:52Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,252,339,712 bytes free, and the
separate `/mnt/cocalc` volume had 63,316,979,712 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,766,458,368 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`819df2923c6758da5abfde9690dc0e5a493de5db`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 08:31 UTC

Read-only reconciliation at `2026-07-27T08:31:40Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,253,679,104 bytes free, and the
separate `/mnt/cocalc` volume had 63,323,766,784 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,995,809,792 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`b0be5fedbef4da705f0c360c5ad21df298f14fb7`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 08:01 UTC

Read-only reconciliation at `2026-07-27T08:01:17Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,246,289,920 bytes free, and the
separate `/mnt/cocalc` volume had 63,330,357,248 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,975,780,352 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`901e2802dfc66c998882536a6bc1560e5bd2b57e`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 07:31 UTC

Read-only reconciliation at `2026-07-27T07:31:30Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,247,682,560 bytes free, and the
separate `/mnt/cocalc` volume had 63,332,229,120 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,804,133,376 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`3f68db304d93ce9c30ef9f06ff90f880ad7c60d4`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 07:01 UTC

Read-only reconciliation at `2026-07-27T07:01:07Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,327,226,880 bytes free, and the
separate `/mnt/cocalc` volume had 63,352,111,104 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,821,729,792 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`f2ce019644e49e78f8a298bb37e3915b9febd2ff`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 06:31 UTC

Read-only reconciliation at `2026-07-27T06:31:33Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,464,246,272 bytes free, and the
separate `/mnt/cocalc` volume had 63,292,014,592 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,829,708,800 bytes free under
`/home/user` and 161,024,090,112 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`636919883df5d0dca8aca2b1beee2872e03154b2`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 06:01 UTC

Read-only reconciliation at `2026-07-27T06:01:17Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,465,487,360 bytes free, and the
separate `/mnt/cocalc` volume had 63,304,339,456 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,836,291,072 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`b442981913bc910cbdede6701c47df41f1e00608`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 05:31 UTC

Read-only reconciliation at `2026-07-27T05:31:51Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,457,909,760 bytes free, and the
separate `/mnt/cocalc` volume had 63,310,630,912 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,828,488,192 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`acc8c18d6a620c940df09d3791c5b8e939e146ae`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 05:01 UTC

Read-only reconciliation at `2026-07-27T05:01:13Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
both durable service names were not found and inactive. Docker was absent,
Podman had no active container, and a literal-stdin process scan found zero
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,459,134,464 bytes free, and the
separate `/mnt/cocalc` volume had 63,320,903,680 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,863,906,304 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`83a3e98c1469fc9a85206c3806425df7ba7aff27`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 04:31 UTC

Read-only reconciliation at `2026-07-27T04:31:54Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,468,653,568 bytes free, and the
separate `/mnt/cocalc` volume had 63,333,318,656 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,873,880,064 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`2c9a30767e550715b5e45f482f62e1cd09dbf6c6`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 04:00 UTC

Read-only reconciliation at `2026-07-27T04:00:59Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,469,865,984 bytes free, and the
separate `/mnt/cocalc` volume had 63,341,240,320 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,882,072,064 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`3ef3937a3bb5d195ddc38ba4b70062d6e7d246bd`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 03:31 UTC

Read-only reconciliation at `2026-07-27T03:31:34Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,471,037,440 bytes free, and the
separate `/mnt/cocalc` volume had 63,350,333,440 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 55,879,659,520 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`22ab6223bc1df0533db411d2392b1021df711e89`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 03:01 UTC

Read-only reconciliation at `2026-07-27T03:01:55Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,462,190,080 bytes free, and the
separate `/mnt/cocalc` volume had 63,353,831,424 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,069,869,568 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`ed4a151fc4598e4a13216fbecae9543630e6f92c`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 02:31 UTC

Read-only reconciliation at `2026-07-27T02:31:07Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,471,787,008 bytes free, and the
separate `/mnt/cocalc` volume had 63,370,682,368 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,094,330,880 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`553753966f297ce5cf73eea2ea32d01476fb0d76`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 02:02 UTC

Read-only reconciliation at `2026-07-27T02:02:11Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,472,925,696 bytes free, and the
separate `/mnt/cocalc` volume had 63,374,614,528 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,100,753,408 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`f85472a447cd02b3ecf771b08f1a8335184c60b1`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 01:31 UTC

Read-only reconciliation at `2026-07-27T01:31:48Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,474,105,344 bytes free, and the
separate `/mnt/cocalc` volume had 63,383,146,496 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,096,276,480 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`31c41165773140324a0d5b6535d2e5fdc10bbd0f`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 01:02 UTC

Read-only reconciliation at `2026-07-27T01:02:14Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,465,991,168 bytes free, and the
separate `/mnt/cocalc` volume had 63,385,477,120 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,097,562,624 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`ac0b3d07f70a385d398068e3c4398768df00a409`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 00:31 UTC

Read-only reconciliation at `2026-07-27T00:31:21Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,467,195,392 bytes free, and the
separate `/mnt/cocalc` volume had 63,398,940,672 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,116,813,824 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`659afa54c3cca49f4f51a106284ebfb5374cbcfd`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-27 Assigned Mount Still Too Small At 00:01 UTC

Read-only reconciliation at `2026-07-27T00:01:23Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,476,448,256 bytes free, and the
separate `/mnt/cocalc` volume had 63,407,202,304 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,138,706,944 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`9e4e348617041f91894307ffdd036e72d6b9b256`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 23:31 UTC

Read-only reconciliation at `2026-07-26T23:31:13Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,411,837,952 bytes free, and the
separate `/mnt/cocalc` volume had 63,414,300,672 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,156,164,096 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`975d60116c0584917cc3765acbd45f113b5395ca`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 23:01 UTC

Read-only reconciliation at `2026-07-26T23:01:38Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,413,005,312 bytes free, and the
separate `/mnt/cocalc` volume had 63,422,226,432 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,153,837,568 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`a6827647e965e94dba34e81f7a0ab0d79b71bc04`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 22:31 UTC

Read-only reconciliation at `2026-07-26T22:31:42Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,405,235,200 bytes free, and the
separate `/mnt/cocalc` volume had 63,428,243,456 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,175,575,040 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`748a86f38a38f70ad51c201968739589cbc2e376`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 22:01 UTC

Read-only reconciliation at `2026-07-26T22:01:49Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,406,472,192 bytes free, and the
separate `/mnt/cocalc` volume had 63,433,449,472 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,192,262,144 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`91528b2a6ed473e40483c01fbd34261494bfa59c`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 21:32 UTC

Read-only reconciliation at `2026-07-26T21:32:28Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,415,987,200 bytes free, and the
separate `/mnt/cocalc` volume had 63,445,884,928 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,207,220,736 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`69cd5435e0a254cc466520d2c8e8d19e920a801c`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 21:01 UTC

Read-only reconciliation at `2026-07-26T21:01:21Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,417,170,944 bytes free, and the
separate `/mnt/cocalc` volume had 63,453,249,536 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,219,062,272 bytes free under
`/home/user` and 161,024,126,976 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`b6eda4e225cbf31560f29e5813de49ed922c8f6e`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 20:31 UTC

Read-only reconciliation at `2026-07-26T20:31:16Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,418,362,880 bytes free, and the
separate `/mnt/cocalc` volume had 63,461,011,456 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,235,851,776 bytes free under
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`04501c9f205c7c99b1b48d59d7ff44bcbdba005d`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 20:02 UTC

Read-only reconciliation at `2026-07-26T20:02:18Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,411,092,480 bytes free, and the
separate `/mnt/cocalc` volume had 63,464,312,832 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,182,878,208 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`d01d44fd045f0bd05c47a6021ccfac89888aa55f`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 19:31 UTC

Read-only reconciliation at `2026-07-26T19:31:40Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,412,374,528 bytes free, and the
separate `/mnt/cocalc` volume had 63,472,459,776 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,206,516,224 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`a7e9fee801bccdc668e9356aedb4041f9a59f153`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 19:01 UTC

Read-only reconciliation at `2026-07-26T19:01:41Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,421,955,072 bytes free, and the
separate `/mnt/cocalc` volume had 63,488,946,176 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,373,489,664 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`53154b48aed8ec25179c51f658252aa57fe21882`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 18:31 UTC

Read-only reconciliation at `2026-07-26T18:31:44Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,423,151,104 bytes free, and the
separate `/mnt/cocalc` volume had 63,493,685,248 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,281,722,880 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`dd5fcd3382fdcc287522a409e1a0d3f4f2706771`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 18:01 UTC

Read-only reconciliation at `2026-07-26T18:01:50Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,424,281,600 bytes free, and the
separate `/mnt/cocalc` volume had 63,496,744,960 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 56,281,837,568 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`aca4df16936e44468a2998e74c9665800dfe2a12`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 17:31 UTC

Read-only reconciliation at `2026-07-26T17:31:49Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,418,444,800 bytes free, and the
separate `/mnt/cocalc` volume had 63,515,213,824 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,430,984,192 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`1f01d59c706ca91494b5650e02968743b523f68b`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 17:01 UTC

Read-only reconciliation at `2026-07-26T17:01:15Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,429,495,808 bytes free, and the
separate `/mnt/cocalc` volume had 63,524,495,360 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,419,175,424 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`16f039c70b32ac6b5bace79459db1c5ed95a8dd4`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 16:32 UTC

Read-only reconciliation at `2026-07-26T16:32:07Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,430,659,072 bytes free, and the
separate `/mnt/cocalc` volume had 63,534,415,872 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,437,529,600 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`81796e397ee4009f8dabfd3a531822086b7ee9a8`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 16:01 UTC

Read-only reconciliation at `2026-07-26T16:01:41Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,433,395,200 bytes free, and the
separate `/mnt/cocalc` volume had 63,544,303,616 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,454,654,976 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`f6bd8d9486f8c9e0b8ef8fc0d1745ca15c09ea72`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 15:31 UTC

Read-only reconciliation at `2026-07-26T15:31:47Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,434,750,976 bytes free, and the
separate `/mnt/cocalc` volume had 63,549,132,800 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,473,635,840 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`bd6155437cfe2ee9f9582f3debd2c4f18fd4249b`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 15:01 UTC

Read-only reconciliation at `2026-07-26T15:01:45Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,444,524,032 bytes free, and the
separate `/mnt/cocalc` volume had 63,550,459,904 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,227,548,160 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`ef7f6af785f62371488d6e9acc515bf5206e4610`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 14:31 UTC

Read-only reconciliation at `2026-07-26T14:31:57Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,445,851,136 bytes free, and the
separate `/mnt/cocalc` volume had 63,564,840,960 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,243,096,576 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`46233e220eaf3ae8da059de2de2cd3467418c5a1`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 14:02 UTC

Read-only reconciliation at `2026-07-26T14:02:05Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,447,198,720 bytes free, and the
separate `/mnt/cocalc` volume had 63,570,280,448 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,234,093,568 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`dcc955a0763b730280e56c9384ed282cf5cd4d8e`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 13:31 UTC

Read-only reconciliation at `2026-07-26T13:31:57Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,448,562,688 bytes free, and the
separate `/mnt/cocalc` volume had 63,576,793,088 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,249,191,424 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`61f07dd537aa57f8a04bff0e2c04226a929f22bd`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 13:01 UTC

Read-only reconciliation at `2026-07-26T13:01:52Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,442,676,736 bytes free, and the
separate `/mnt/cocalc` volume had 63,585,869,824 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,260,488,192 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`b5a4a0a2a88de3b9b5d4d0705565c180c5b59b62`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 12:32 UTC

Read-only reconciliation at `2026-07-26T12:32:08Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,444,007,936 bytes free, and the
separate `/mnt/cocalc` volume had 63,595,941,888 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,267,320,320 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`ee2cc62d964a2d2d5fe84019a98032d17b3df0cf`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 12:01 UTC

Read-only reconciliation at `2026-07-26T12:01:37Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,453,768,704 bytes free, and the
separate `/mnt/cocalc` volume had 63,598,157,824 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,280,632,320 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`fe71dc9ef481bd8d5c77a12de4e0ad1387ef18d1`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 11:32 UTC

Read-only reconciliation at `2026-07-26T11:32:37Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,455,063,040 bytes free, and the
separate `/mnt/cocalc` volume had 63,611,944,960 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,294,890,496 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`d63e99ac628843767792d9dbc75b3251fb5d60bb`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 11:01 UTC

Read-only reconciliation at `2026-07-26T11:01:56Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,456,427,008 bytes free, and the
separate `/mnt/cocalc` volume had 63,617,204,224 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,306,768,896 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`d13cd20e6e801abfee2a87cb5010e5fa7b972987`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.

## 2026-07-26 Assigned Mount Still Too Small At 10:31 UTC

Read-only reconciliation at `2026-07-26T10:31:35Z` reached `host` on the
first bounded attempt as native Linux `x86_64`. The assigned
`/mnt/cocalc-scratch` path remained backed by the 52,521,566,208-byte
`/dev/sdc` ext4 filesystem with 49,820,409,856 bytes free. Its only entry was
`lost+found`, the authoritative exact-source run root remained absent, and
...
The assigned filesystem cannot satisfy the binary 100 GiB heavy-build
threshold even when empty, so no cleanup can make it suitable for the
`post64` rebuild. The root filesystem had 17,443,794,944 bytes free, and the
separate `/mnt/cocalc` volume had 63,628,791,808 bytes free, also below the
threshold and outside the assigned automation root. The controller
filesystems used by this iteration had 58,321,367,040 bytes free under
`/home/user` and 161,024,131,072 bytes free under `/scratch`.
...
artifact. Before this evidence edit, the canonical checkout, its tracking
ref, and the directly queried `origin/develop` ref were synchronized at
`1e1c076c9216d35858c3f8b40da846d5a055b8e4`. Exact pushed `post64` source
`014ae4bf44318b6f5032053957a92291d4363b7a` remains an ancestor of that
branch.
