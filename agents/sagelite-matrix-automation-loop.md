...
| Linux aarch64 | 3.13 | yes (`post32`, local) | exact pushed `post32` source `7779ed60f7b` built successfully and passed strict wheelhouse preflight, fresh wheel-only installation, `pip check`, selftest, FriCAS, GAP3, and QEPCAD. The short sweep doctested 3,953 files and failed only the two FLINT polynomial-power examples in one module. A focused exact-seed run proved the trigger is upstream `pplpy 0.9.0` exporting its statically embedded GMP into Sagelite's process; a repaired `pplpy 0.9.0.post1` dynamically linked to Sagelite's PPL/GMP passed the trigger and all 2,848 polynomial-element doctests. Exact pushed `post33` sources `c615947f2bc`, `ddf660b7c87`, and `17c0fb4d05e` built raw primaries but failed during repair because the pplpy phase selected an incomplete build frontend through the layered Sage-prefix interpreter. Exact pushed source `f67e0eadcb7` pins repair phases to cibuildwheel's absolute interpreter path; its single native replacement build is healthy in wheel construction with 1,101 Ninja outputs recorded, and its gated watcher is waiting, with no wheel or gate result claimed yet | none |
