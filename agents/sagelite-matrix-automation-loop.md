...
|---|---:|---|---|---|
| Linux x86_64 | 3.12 | yes (`post9` public); exact `post60` job unreconciled | full baseline; 3,953 modules and 0 failures on the earlier accepted build. Read-only preflight at `2026-07-20T21:01:31Z` reached native `x86_64`, but the assigned `/mnt/cocalc-scratch` path is an essentially empty 20,957,446,144-byte ext4 filesystem with only 19,866,902,528 bytes free, far below the binary 100 GiB heavy-build threshold. The recorded exact `post60` run remains absent. The separate `/mnt/cocalc` filesystem has 212,582,916,096 bytes free but is not the assigned build root; explicit direction or restoration of a qualifying assigned mount is required | smoke (`post8`) |
