...
| macOS arm64 | 3.13 | yes (`post39`, local; `post9`, public) | smoke only; exact pushed source `c6c8d1b6665` produced a portable local `post39` primary whose strict repair audited 1,178 dependencies across 638 Mach-O files with no disallowed load paths. Exact pushed source `cc8d999d436` produced the 3 previously missing base companion wheels; each passed an exhaustive Mach-O audit and fresh isolated runtime smoke. Exact pushed sources `7d63944caeb` and `2df580c98da` produced 7 more portable companions; exact pushed validation source `dd2cf612b01` completed their fresh isolated smokes, with 99 audited Mach-O files in total and no disallowed load paths. Exact pushed source `099c31b802a` produced a portable CSDP wheel whose 5 Mach-O files passed strict audit, fresh wheel-only installation, `pip check`, and a functional theta smoke. Exact pushed source `4075521634d` produced a portable Giac wheel whose 20 Mach-O files and 71 dependencies passed strict audit, fresh wheel-only installation, `pip check`, and version and arithmetic smokes. Exact pushed sources `300764eac33` and `55949b5b79a` produced portable lrslib, msolve, and QEPCAD wheels; their 12 Mach-O files and 40 dependencies passed strict audit, fresh wheel-only installation, `pip check`, and functional or repeated startup smokes. Exact pushed source `b70bf3d81af` produced the 9,100,370,523-byte platform-independent 4D polytope database wheel; its 17,975-file payload passed a fresh wheel-only installation, `pip check`, data-path checks, and entry-point smoke. Exact pushed source `d0f1d2296ea` produced a portable Kenzo wheel whose 4 Mach-O files and 14 dependencies passed strict audit, fresh wheel-only installation, `pip check`, shared companion smoke, and neutral `dlopen` smoke. Exact pushed source `af6c607fe0a` produced a portable GNU Info wheel whose arm64 Mach-O and both dependencies passed strict audit, fresh wheel-only installation, `pip check`, and a functional Singular Info-node smoke. Exact pushed source `a754d6ae457` produced a portable FriCAS wheel whose 24 Mach-O files and 76 dependencies passed strict audit, fresh wheel-only installation, `pip check`, shared companion smoke, and a functional factorization smoke. Exact pushed source `b4fb12e6c62` produced the final missing ImageMagick companion; its 149 Mach-O files and 1,150 dependencies passed strict audit, fresh wheel-only installation, `pip check`, and neutral PPM-to-PNG-to-GIF conversion. Every individually missing companion now has a compatible local wheel, but final coherent-revision validation must rebuild the primary from the selected current commit, assemble the full closure, and run the strict short and full gates | smoke (`post8`) |
...
1. Build the remaining macOS-compatible `all-needed-extras` companion
   distribution identified in
   `agents/sagelite-macos-arm64-cp313-validation.md`, rebuild the primary from
   the selected coherent source revision, assemble the complete wheelhouse,
   then run strict clean-install, short, and full validation for macOS arm64
   CPython 3.13.
