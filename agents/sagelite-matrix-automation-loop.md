...
| Linux aarch64 | 3.13 | yes (`post22`, local); `post23` rebuild active | the exact committed `post22` primary and matching Maxima wheel built natively. Its fresh strict 178-wheel closure passed every preflight, wheel-only installation, `pip check`, runtime manifest, and selftest probe, but the short sweep failed 5 of 3,954 modules. A focused `post23` probe packages the missing native headers and passes all 115 focused tests plus all 48 `sage.misc.cython` doctests at committed SHA `9ca961f7c25`; the authoritative exact-SHA native rebuild is active with one durable validation watcher queued to run the short and full gates after build success, so no new wheel or suite pass is claimed | none |
