...
| Linux aarch64 | 3.13 | yes (`post27`, local); `post28` source ready for rebuild | the exact committed `post27` primary passed every strict wheelhouse preflight, wheel-only installation, and `pip check`, but the strengthened QEPCAD selftest returned an empty answer. The Singular discovery helper loaded the companion runtime without its real package context, so its `importlib.resources` lookup failed and the intended child `PATH` was never added. The `post28` correction imports the optional runtime with its package context; 38 focused unit tests and the real Singular-backed selftest passed against the untouched failed install. No `post27` smoke or full-suite pass is claimed | none |
