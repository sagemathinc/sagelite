...
| Linux aarch64 | 3.14 | yes (`post36`, local); exact pushed stable-interpreter source `c483e52c63c5` produced a repaired primary and three rebuilt companions. A host-mount path mismatch made the outer collector exit 1 after the artifacts were complete; they are hash-inventoried in a separate recovery run. Exact pushed `post37` source `09cc51c9c452` fixes that staging contract for the next normal build | `post33` strict closure, wheel-only install, `pip check`, all 102 selftest probes, and packaged pytest passed; its short doctest gate failed because the primary used CPython 3.14.0b3 private headers. The recovered stable-interpreter `post36` 167-wheel closure is rerunning the gated short/full validator; no short or full pass is claimed yet | none |
