...
| Linux aarch64 | 3.14 | missing; three native `post33` builds failed before producing wheels after exposing stale cross-Python configuration, venv-interpreter state, and pure-Python build-tool markers; corrected exact-SHA rebuild pending | none; no wheel or validation result claimed yet | none |
