...
| Linux aarch64 | 3.13 | yes (`post19`, local) | the exact committed `post19` primary and matching Maxima wheel built successfully. Its fresh strict 177-wheel installation, `pip check`, every selftest probe, and packaged pytest passed natively, but the 3,954-module `--optional=sage` short sweep failed 13 modules: eleven core-supported and two optional-external. Exact pushed `post20` source `87330f2482d5fd788cfb737c4a7c4f4f32c06301` corrects one coherent five-example result-rendering class across msolve, polynomial ideals, and Boolean polynomial sequences; all 1,354 focused doctests pass against the installed `post19` runtime. Its native exact-SHA rebuild and gated short/full watcher are active. No `post20` wheel is claimed, and the cell remains below `full` | none |
