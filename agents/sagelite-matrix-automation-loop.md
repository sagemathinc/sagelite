...
| Linux aarch64 | 3.13 | yes (`post15`, local) | the exact committed `post15` primary and matching Maxima wheels built successfully. Its fresh strict 177-wheel gate passed wheel-only installation, `pip check`, every selftest probe, all 3,953 short-sweep modules, and packaged pytest. The subsequent full sweep has confirmed failures and remains active only to collect its complete reduced inventory: the Giac 1.9 conditional expectation referenced a helper outside the doctest namespace, and an exact installed probe proved the flatter companion omitted `libgomp.so.1`. Committed `post16`/flatter `post1` source `39fd8bb94c90c7249ff8d1f80a13d2ab9a93cc46` fixes both failures; a durable exact-SHA follow-on is waiting for the complete `post15` inventory before rebuilding, and the cell remains below `full` | none |
