...
| Linux aarch64 | 3.13 | yes (`post29`, local); `post30` rebuild active | exact committed `post29` source `37e1224bcce` built successfully and passed strict wheelhouse preflight, fresh wheel-only installation, `pip check`, and selftest, including the repaired 3,000,000-cell QEPCAD regression. Packaged pytest passed 215 tests with two skipped. The 3,954-module short sweep failed four modules: FriCAS, GAP3, two FLINT polynomial-power examples, and two stale QEPCAD command-string expectations. Exact pushed `post30` source `7cd5cb5f82d` makes those expectations companion-aware; all 344 QEPCAD doctests and four focused unit tests passed against the untouched `post29` wheel-only install. One native `post30` rebuild and gated watcher are active; no short or full pass is claimed | none |
