...
| Linux aarch64 | 3.13 | yes (`post19`, local) | the exact committed `post19` primary and matching Maxima wheel built successfully. Its fresh strict 177-wheel installation, `pip check`, and every selftest probe passed natively. The installed 3,953-module `--optional=sage` short gate is actively running across eight workers; no failures have been emitted so far, but neither a short nor full pass is claimed before completion and reduced analysis. The cell remains below `full` | none |
