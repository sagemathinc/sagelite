...
| Linux aarch64 | 3.13 | yes (`post15`, local) | the exact committed `post15` primary and matching Maxima wheels built successfully. Its fresh strict 177-wheel gate passed wheel-only installation, `pip check`, every selftest probe, all 3,953 short-sweep modules, and packaged pytest. Its complete full sweep finished with 24 failed modules out of 3,958: 14 core-supported, seven timeout/performance, two optional-external, and one optional-data. Committed `post16`/flatter `post1` source `39fd8bb94c90c7249ff8d1f80a13d2ab9a93cc46` fixes the selected Giac namespace and missing `libgomp.so.1` failures; its durable exact-SHA native rebuild is active in wheel construction, and the cell remains below `full` | none |
