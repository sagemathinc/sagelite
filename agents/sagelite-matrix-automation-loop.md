...
| Linux aarch64 | 3.13 | yes (`post13`, local) | exact-SHA repaired primary plus matching ImageMagick `post2`, Maxima `post15`, and msolve `post2` built; the strict 177-wheel short gate passes fresh wheel-only install, `pip check`, every selftest probe, 3,953 installed doctest modules with 0 failures, and packaged pytest with 213 passed and 2 skipped. The full installed `--optional=sage` validation is running from the same wheelhouse | none |
