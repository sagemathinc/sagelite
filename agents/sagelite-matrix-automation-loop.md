...
| Linux aarch64 | 3.13 | yes (`post25`, local); `post26` rebuild active | the exact committed `post25` primary and nominally rebuilt QEPCAD `post1` companion passed every strict wheelhouse preflight, wheel-only installation, and `pip check`; packaged pytest passed 215 tests with two skips. Selftest and the short sweep still exposed QEPCAD `bad_alloc`, and the sweep failed 4 of 3,954 modules: QEPCAD, GAP3, polynomial exponentiation, and one Cython doctest. Debug symbols proved the `post1` binary still used `char`: the persistent manylinux prefix's unchanged `qepcad-1.74` install stamp had skipped recompilation and repackaged the old executable. Committed `post26` source `6d670ebaaa3` advances the source package to `1.74.p1` and the companion to `post2`; its exact-SHA native rebuild and one gated watcher are active, so no `post26` wheel or suite pass is claimed | none |
