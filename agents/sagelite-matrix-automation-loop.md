...
| Linux aarch64 | 3.13 | yes (`post16`, local) | the exact committed `post16` primary, matching Maxima wheel, and focused flatter `post1` companion built successfully from source `39fd8bb94c90c7249ff8d1f80a13d2ab9a93cc46`. The replacement flatter wheel bundles `libgomp.so.1` and passes clean native runtime smoke. The corrected fresh strict 177-wheel gate passed wheel-only installation, `pip check`, runtime collection, every selftest probe, all 3,953 installed `--optional=sage` short-sweep modules, and packaged pytest. A fresh full sweep from the same wheel contract is active with eight native workers. The preceding `post15` full sweep finished with 24 failed modules out of 3,958: 14 core-supported, seven timeout/performance, two optional-external, and one optional-data. The cell remains below `full` | none |
