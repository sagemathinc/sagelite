...
| Linux aarch64 | 3.13 | yes (`post31`, local) | exact committed `post31` source `b50050c219c` built successfully and passed strict wheelhouse preflight, fresh wheel-only installation, `pip check`, and selftest. The corrected process-private FriCAS helper-compilation directory eliminated the earlier concurrent conversion failure. The short sweep failed only GAP3 and two FLINT polynomial-power examples: the runner doctested 3,953 files and the reducer recorded two failed modules out of 3,954. Exact pushed `post32` source `7779ed60f7b` fixes the GAP3 name-lifecycle, help-protocol, and matrix-LaTeX failures; five focused GAP3 runs, the generic GAP suite, and the isolated GAP3 selftest passed against the untouched `post31` wheel install with a read-only source overlay. Its native exact-SHA rebuild and gated validation are running; no `post32` wheel or gate result is claimed yet | none |
