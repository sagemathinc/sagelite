...
| Linux aarch64 | 3.13 | yes (`post28`, local); `post29` rebuild required | exact committed `post28` source `f239768d9a1` built successfully and passed strict wheelhouse preflight, fresh wheel-only installation, `pip check`, and selftest. The 3,954-module short sweep failed 13 examples: eight independent GAP3 protocol failures, two independent FLINT aborts, and three QEPCAD failures. The complex QEPCAD case exposed SACLIB's aarch64 conservative GC omitting live handles in callee-saved registers. Focused SACLIB `2.2.8.p1` and QEPCAD `1.74.p2` builds now spill x19-x28 and scan the saved caller frame; the exact 3,000,000-cell regression passed 10/10 in the native manylinux environment. Sagelite `post29` and QEPCAD runtime `post3` still need an exact-SHA rebuild and strict gates, so no smoke or suite pass is claimed | none |
