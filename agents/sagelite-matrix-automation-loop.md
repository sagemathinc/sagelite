...
| Linux aarch64 | 3.14 | missing; two native `post33` builds failed before producing wheels after exposing stale cross-Python configuration and venv-interpreter state; corrected exact-SHA rebuild active | none; no wheel or validation result claimed yet | none |
