...
| Linux aarch64 | 3.14 | yes (`post33`, local); exact pushed source `5ab41cd4b684` produced a repaired primary plus the missing CPython 3.14 `cysignals` and `pycosat` wheels. The stable-interpreter `post34` retry reached a repaired primary but cibuildwheel 3.4.1 rejected multiple repair outputs and retained no wheel. Exact pushed `post35` source `a32742fac4a` is rebuilding with separate primary and companion destinations | `post33` strict closure, wheel-only install, `pip check`, all 102 selftest probes, and packaged pytest passed; the short doctest gate failed because the primary used CPython 3.14.0b3 private headers. The native `post35` build and gated short/full watcher are active; no `post35` wheel or validation result is claimed yet | none |
