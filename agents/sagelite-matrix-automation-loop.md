...
| Linux aarch64 | 3.13 | yes (`post24`, local) | the exact committed `post24` primary and matching Maxima wheel built natively. Its fresh strict 178-wheel closure passed every preflight, wheel-only installation, `pip check`, runtime manifest, selftest, and 215 packaged pytest tests. The FPLLL relocation failure is fixed, but the short sweep failed 3 of 3,954 modules: QEPCAD startup, GAP3 protocol behavior, and polynomial exponentiation aborts. The gated watcher correctly did not start the full sweep, so no suite pass is claimed | none |
