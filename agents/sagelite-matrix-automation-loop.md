...
| Linux aarch64 | 3.14 | yes (`post33`, local); exact pushed source `5ab41cd4b684` produced a repaired primary plus the missing CPython 3.14 `cysignals` and `pycosat` wheels. The stable-interpreter `post34` retry reached a repaired primary but cibuildwheel 3.4.1 rejected multiple repair outputs. The `post35` retry separated those outputs but its Maxima companion step still looked for the primary in the companion directory, so cibuildwheel retained no wheel. Exact pushed `post36` source `c483e52c63c5` is rebuilding with the corrected primary lookup | `post33` strict closure, wheel-only install, `pip check`, all 102 selftest probes, and packaged pytest passed; the short doctest gate failed because the primary used CPython 3.14.0b3 private headers. The native `post36` build and gated short/full watcher are active; no `post35`/`post36` wheel or validation result is claimed | none |
