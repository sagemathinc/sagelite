...
|---|---:|---|---|---|
| Linux x86_64 | 3.12 | yes (`post9` public); exact `post60` job unreconciled | full baseline; 3,953 modules and 0 failures on the earlier accepted build. Three bounded SSH attempts to the required alias timed out, with the final attempt ending at `2026-07-20T15:01:51Z`; no later remote state is inferred. The last reachable preflight at `2026-07-20T03:31:15Z` found `/mnt/cocalc-scratch` backed by an essentially empty 20,957,446,144-byte filesystem with only 19,866,902,528 bytes free. The recorded exact `post60` run was absent, so no synchronized result is inferred without the bulk artifacts. A separate `/mnt/cocalc` filesystem then had 207,010,279,424 bytes free but is not the assigned build root; explicit direction or restoration of a qualifying assigned mount is required | smoke (`post8`) |
