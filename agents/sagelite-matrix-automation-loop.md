...
| Linux aarch64 | 3.13 | yes (`post30`, local); `post31` rebuild active | exact committed `post30` source `7cd5cb5f82d` built successfully and passed strict wheelhouse preflight, fresh wheel-only installation, `pip check`, and selftest. Packaged pytest passed 215 tests with two skipped. The QEPCAD failures are gone; the 3,954-module short sweep failed only FriCAS, GAP3, and two FLINT polynomial-power examples. Exact pushed `post31` source `b50050c219c` gives each FriCAS process a private helper-compilation directory; the unmodified `post30` code failed all eight concurrent focused conversions, while the exact corrected source passed all eight against the untouched wheel-only install. Its sole native rebuild is healthy in extension compilation and one gated watcher is waiting; no `post31` wheel, short pass, or full pass is claimed | none |
