...
| Linux aarch64 | 3.13 | yes (`post24`, local); `post25` rebuild active | the exact committed `post24` primary and matching Maxima wheel built natively. Its fresh strict 178-wheel closure passed every preflight, wheel-only installation, `pip check`, runtime manifest, selftest, and 215 packaged pytest tests. The FPLLL relocation failure is fixed, but the short sweep failed 3 of 3,954 modules: QEPCAD startup, GAP3 protocol behavior, and polynomial exponentiation aborts. A debugger identified QEPCAD's aarch64 EOF signedness bug; committed `post25` source `185c5adec4c` preserves EOF, requires a freshly rebuilt `post1` companion on every ABI, and adds real startup gates. The exact-SHA native rebuild and one gated validation watcher are active, so no `post25` wheel or suite pass is claimed | none |
