...
| Linux aarch64 | 3.13 | yes (`post23`, local); `post24` rebuild active | the exact committed `post23` primary and matching Maxima wheel built natively. Its fresh strict 178-wheel closure passed every preflight, wheel-only installation, `pip check`, runtime manifest, selftest, and 215 packaged pytest tests, but the short sweep failed 3 of 3,954 modules: FPLLL strategy relocation, QEPCAD startup, and GAP3 protocol behavior. A focused `post24` probe fixes FPLLL's private `BKZ.EasyParam` path copy and passes all 152 `sage.modules.free_module_integer` doctests at committed SHA `bd7e96a663b`; the authoritative exact-SHA native rebuild is actively compiling with one durable validation watcher queued to run the short and full gates after build success, so no `post24` wheel or suite pass is claimed | none |
