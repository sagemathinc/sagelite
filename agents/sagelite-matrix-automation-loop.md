...
| Linux x86_64 | 3.13 | exact `post64` portable rebuild active; `post60` local and `post9` public rejected for CPU portability | rebuild active. After CP312 full acceptance, its completed 21 GB install venv was removed while preserving validated artifacts, restoring 144 GiB free. The staged exact-source archive, clean checkout, fat prefix, Docker-idle state, and four launcher hashes passed. Durable build and guarded watcher units started at `2026-07-30T02:14:22Z`; the watcher pins the accepted CP313 `pycosat` wheel and requires the corrected Nehalem probe plus fresh short/full gates. No wheel or pass is claimed yet | smoke (`post8`), now rejected for CPU portability |
