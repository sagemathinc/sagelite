...
| Linux aarch64 | 3.13 | yes (`post27`, local); `post28` rebuild active | the exact committed `post27` primary passed every strict wheelhouse preflight, wheel-only installation, and `pip check`, but the strengthened QEPCAD selftest returned an empty answer. The Singular discovery helper loaded the companion runtime without its real package context, so its `importlib.resources` lookup failed and the intended child `PATH` was never added. Committed `post28` source `f239768d9a1` imports the optional runtime with its package context; 38 focused unit tests and the real Singular-backed selftest passed against the untouched failed install. Its exact-SHA native rebuild is healthy at compiled extension target 1,105 of 1,794, and one gated watcher is waiting, so no `post28` wheel or suite pass is claimed | none |
