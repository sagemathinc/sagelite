...
| Linux x86_64 | 3.13 | yes (`post64`, local); exact portable primary is 261,575,877 bytes with SHA256 `ae5624131009064ca0ed34378f5b19c9a2eea677219b5693e6315ef763f82589`; `post60` local and `post9` public rejected for CPU portability | short (`post64`), fresh full rerun active. The exact build and strict 192-wheel closure passed the corrected fresh Nehalem probe. The independent short gate passed. The first full gate passed installation, isolation, every selftest, and packaged pytest but rejected one performance-only worker timeout in `sage.matrix.matrix_integer_dense` with zero failed examples. Its exact-seed isolated replay passed the complete module in 12.6 seconds. A separately named fresh full rerun from the unchanged closure started at `2026-08-01T20:31:09Z`; full acceptance remains pending | smoke (`post8`), now rejected for CPU portability |
