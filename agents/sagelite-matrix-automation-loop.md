...
| Linux aarch64 | 3.14 | yes (`post36`, local); exact pushed stable-interpreter source `c483e52c63c5` produced a repaired primary and three rebuilt companions, recovered and hash-inventoried after an outer collector path mismatch. Exact pushed `post37` source `09cc51c9c452` fixes companion staging. Exact pushed `post38` source `a2bdbbb674e` removes the remaining CPython 3.14 private interpreter-state offset and is rebuilding natively | the recovered `post36` 167-wheel strict closure passed preflight, wheel-only install, `pip check`, all 102 selftest probes, and packaged pytest (215 passed, 2 skipped), but all doctest workers still segfaulted because the primary built on CPython 3.14.3 embedded a private offset changed by 3.14.6. The focused `post38` cross-patch extension test passes; no short or full pass is claimed yet | none |
