...
| Linux aarch64 | 3.13 | yes (`post26`, local); `post27` rebuild active | the exact committed `post26` primary and genuinely rebuilt QEPCAD `post2` companion passed every strict wheelhouse preflight, wheel-only installation, `pip check`, and startup selftest. The `bad_alloc` is fixed, but the short sweep exposed a second QEPCAD integration defect: its `SINGULAR yes` child could not find the companion's private uppercase executable, died with `SIGPIPE`, logged 66 protocol failures, and timed out; GAP3 remained a separate failed module. A focused run proved that exposing the companion bindir makes `qepcad(a > b)` return `b - a < 0`. Committed `post27` source `b9c7d52af26` adds that directory only to the QEPCAD child environment and strengthens selftest to require the Singular-backed operation. Its exact-SHA native rebuild has reached active extension compilation with one gated watcher waiting, so no `post27` wheel or suite pass is claimed | none |
