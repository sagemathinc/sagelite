...
def is_singular_runtime_dependency(path: str) -> bool:
    name = Path(path).name
    return name.startswith(SINGULAR_RUNTIME_PREFIXES)


def repair_sage_extensions(root: Path) -> int:
    runtime_lib_dir = root / "sagelite_singular_runtime" / "data" / "lib"
    changed = 0
    for extension in sorted((root / "sage").rglob("*.so")):
        ...
        for dependency in otool_libraries(extension):
            if is_singular_runtime_dependency(dependency):
                args.extend(
                    [
                        "-change",
                        dependency,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Rewrite macOS sagelite Singular extension modules to load "
            "libSingular and related libraries from the sagelite-singular-runtime "
            "companion package."
        )
    )
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        changed = repair_sage_extensions(root)
        if changed == 0:
            raise SystemExit(
                "no Sage extension modules referenced Singular runtime libraries"
            )
        pack_wheel(root, args.out)
        print(f"repaired {changed} Sage extension modules")
